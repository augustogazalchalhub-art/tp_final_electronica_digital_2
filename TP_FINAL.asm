...
#INCLUDE <P16F887.INC>
    ...
    ;cosas a modificar:
    ;multiplicar en bcd no en binario
    ;fijarse que ne la comparacion las unidades entran con valores de teclado
    ;la comparacion hacerla en bcd
    ;revisar la logica de los estados
    

ESTADO		    EQU 0X20 ; ( flag lleno,teclado o llenando, antirrebote, espera)
...
START: 
    ...
    BCF     PIE1, RCIE      ; Deshabilitamos la interrupción de recepción
    ...
    BANKSEL ADCON1
    MOVLW   0X10
    MOVWF   ADCON1		; VREF+ = 4,24V, VREF- = VSS, justificación izquierda
...
ENVIAR_CARACTER:
    BANKSEL TXSTA
  ESPERO_TX:
    BTFSS   TXSTA, TRMT		; ¿Buffer vacío?
    GOTO    ESPERO_TX		; No, sigue esperando
...
ENVIAR_MENSAJE:
  ...
  LOOP_MENSAJE:
    ...
    MOVWF   PCL			; Voy a la tabla y vuelvo con el carácter en W
    
    ; Comprueba si es el fin de string
    ADDLW   0X00		; Solo para afectar el flag Z
    BTFSC   STATUS, Z		; ¿Fue W = 0?
    RETURN			; Sí, fin del mensaje. Retorna.
    CALL    ENVIAR_CARACTER	; No fue 0, así que envía el carácter
    
    INCF    INDICE_UART, F	; Incrementa el indice al siguiente carácter
    GOTO    LOOP_MENSAJE
...
ADDWF   PCL,F
    ...
    RETLW   B'01111111'      ; 8
    RETLW   B'01101111'      ; 9
    RETLW   B'00111001'      ; C (RESET)
    RETLW   B'01111001'	     ; E
    RETLW   B'00111111'      ; 0
    RETLW   B'01110001'	     ; F
    RETLW   B'01011110'	     ; D (ENTER)
...
ISR:
  ...
  ISR_TIMER:
    ...
    GOTO    NIVEL_ALCANZADO
NIVEL_ALCANZADO:
    BTFSC ESTADO, 3 ; FLAG_ MSJ ENVIADO
    GOTO FIN_ADC_ISR
    BSF ESTADO, 3 ; FLAG ENVIADO
    ;BCF PORTE, 0
    BCF PORTE, 1
